...
import io.shaderfoundry.lib.program.ProgramPipeline
    ...
    internal fun createPipelines(
        ...
        return InputFormat.entries
            .associate { inputFormat ->
                ...
                val pipeline = when {
                    inputFormat == InputFormat.DEFAULT -> ProgramPipeline.Companion()
                    inputFormat == InputFormat.YUV -> ProgramPipeline.Companion()
                    inputFormat == InputFormat.UNKNOWN -> ProgramPipeline.Companion()
                    dynamicRange.is10BitHdrBackport -> ProgramPipeline.Companion()
                    else -> ProgramPipeline.Companion()
                }
